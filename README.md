...
<details>
  ...
  Опции:
   ...
   - `kks-ssh-timeout` (по умолчанию `5`) - таймаут для команд, отправляемых через SSH
...
Для подключения к SSH **не нужно** вручную заходить на сервер и запускать ejudge-fuse, всё происходит автоматически при запуске команды `kks ssh`. После успешного выполнения команды ejudge-fuse на сервере будет смонтирована в директорию, указанную в опции `--mnt-dir` (по умолчанию `/tmp/ejudge_fuse_mnt`)
