...
def get_top1_score(standings):
    ...
    for row in standings.rows:
        ...
        for task in row.tasks:
            if not is_bonus_contest(task.contest) and task.score is not None:
...
def is_bonus_contest(contest):
    return (not contest.startswith("kr")) and (not contest.startswith("sm")) and (not contest.startswith("ku"))
