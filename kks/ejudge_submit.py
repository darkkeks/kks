...
class SubmissionResult:
    ...
    def parse_status(cls, run_status):
        ...
        # PENDING == "Pending check" =?= ACCEPTED (??)
        if run_status.status in [RunStatus.ACCEPTED, RunStatus.PENDING]:
        ...
        # Hide OK's. There can be 100+ passed tests and a few failed.
        return cls.fail(run_status.with_tests(failed_only=True))
...
def submit_solution(session, file, prob_name, timeout):
    ...
    if (
        any(field in problem_status for field in ok_fields)
